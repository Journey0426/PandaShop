  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="info in addressInfo":key="info.id">
        <span class="username" :class="{selected:info.isDefault==1}">{{info.consignee}}</span>
        <p @click="changeDefault(info,addressInfo)">
    ...
    <div class="money clearFix">
      <ul>
        <li>
          <b><i>{{orderinfo.totalNum}}</i>种商品，总商品金额</b>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{Enduseraddress.fullAddress}}</span>
        收货人：<span>{{Enduseraddress.consignee}}</span>
        <span>{{Enduseraddress.phoneNum}}</span>
      </div>
    </div>
...
import Login from "@/pages/Login";
  export default {
    ...
    mounted() {
      ...
      //购物清单
      this.$store.dispatch('trade/getShopcart')
    },
    computed:{
      //地址
      ...mapState({addressInfo:state => state.trade.address}),
      //商品信息
      ...mapGetters({orderinfo:'trade/orderinfo'}),
      //将来提交订单最终选中的地址
      Enduseraddress(){
        //find找到符合条件的元素
    ...
    methods:{
      //排他操作
      changeDefault(info,allinfo){
        //排他操作
        //把全部的属性值isdefault属性变为0
        //只有点击的属性值才是1
      ...
      async submitOrder(){
        ...
        let data = {
          consignee: this. Enduseraddress.consignee, //收件人的名字
          consigneeTel: this. Enduseraddress.phoneNum, //收件人得电话
          deliveryAddress: this. Enduseraddress.fullAddress, //收件人地址
